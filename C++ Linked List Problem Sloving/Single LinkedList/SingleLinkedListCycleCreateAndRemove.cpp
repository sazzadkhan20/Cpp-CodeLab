...
void cycle_create_in_linkedList(node* &head,int position)
{
    if(head==0)
    {
        cout<<"Linked List Empty\n\n";
...
bool cycle_detection_in_linkedList(node* head)
{
     if(head==0)
    {
        cout<<"Linked List Empty\n\n";
...
void cycle_remove_in_linkedList(node* head)
{
    if(head==0)
    {
        cout<<"Linked List Empty\n\n";
    ...
    if(cycle_detection_in_linkedList(head))
    {
       node* fast = head;
       node* slow = head;
...
int main()
{
    ...
    reverse_Node(head);
    node_display(head);
    cycle_create_in_linkedList(head,40);
    //node_display(head);
    if(cycle_detection_in_linkedList(head))
    {
        cout<<"Cycle Present In Linked List\n\n";
    }
    cycle_remove_in_linkedList(head);
    node_display(head);
    cout<<cycle_detection_in_linkedList(head)<<endl;
    delete head;
}
